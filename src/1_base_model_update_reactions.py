...
# runs test suite with iEK1008.json
run_test_suite('../models/iEK1008.json')
...
# removes ten imbalanced reactions from iEK1008; all ten reactions are added back with balanced formulas during update
...
with alive_bar(len(df_new_rxns), bar='blocks', spinner='notes_scrolling') as bar:
    for index, row in df_new_rxns.iterrows():
        ...
        new_rxn = create_reaction(model, row['Reaction_ID'], row['Reaction_Name'], row['Subsystem'], new_rxn_mets,
